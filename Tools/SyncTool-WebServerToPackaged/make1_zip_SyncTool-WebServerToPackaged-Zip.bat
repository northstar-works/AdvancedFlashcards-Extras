...
REM Put this BAT in: tools\SyncTool-WebServerToPackaged\
REM or:              Tools\SyncTool-WebServerToPackaged\
REM
REM Creates ZIP in:  <project_root>\logs\zips\
REM   SyncTool-WebServerToPackaged-v<ver>_b<build>.zip
REM ZIP contents root folder:
REM   SyncTool-WebServerToPackaged-v<ver>_b<build>\...
REM Reads:
REM   .\version.json
REM Excludes (recursive):
REM   output, logs, sync_backups, .venv, __pycache__, *.pyc, *.log, *.zip
REM ============================================================

REM Always run from the BAT's folder (tool folder)
set "TOOL_DIR=%~dp0"
if "%TOOL_DIR:~-1%"=="\" set "TOOL_DIR=%TOOL_DIR:~0,-1%"
pushd "%TOOL_DIR%" >nul

REM Project root is TWO levels up from ...\tools\SyncTool-...
for %%I in ("%TOOL_DIR%\..\..") do set "PROJECT_ROOT=%%~fI"

REM Output dir for zips and zip logs
set "OUTDIR=%PROJECT_ROOT%\logs\zips"
if not exist "%OUTDIR%" mkdir "%OUTDIR%" >nul 2>&1

set "LOG=%OUTDIR%\make1_zip_SyncTool-WebServerToPackaged.log"
> "%LOG%" echo START %DATE% %TIME%
>>"%LOG%" echo TOOL_DIR=%TOOL_DIR%
>>"%LOG%" echo PROJECT_ROOT=%PROJECT_ROOT%
...
REM Naming (as requested)
set "ROOTNAME=SyncTool-WebServerToPackaged-v%VER%_b%BUILD%"
set "ZIPNAME=%ROOTNAME%.zip"
set "ZIPPATH=%OUTDIR%\%ZIPNAME%"
>>"%LOG%" echo ROOTNAME=%ROOTNAME%
>>"%LOG%" echo ZIPPATH=%ZIPPATH%

if exist "%ZIPPATH%" del /f /q "%ZIPPATH%" >>"%LOG%" 2>&1

REM ---------- STAGING COPY ----------
set "STAGE=%TOOL_DIR%\_zip_stage"
>>"%LOG%" echo STAGE=%STAGE%

if exist "%STAGE%" rmdir /s /q "%STAGE%" >>"%LOG%" 2>&1
mkdir "%STAGE%\%ROOTNAME%" >>"%LOG%" 2>&1

REM Copy THIS tool folder into stage\ROOTNAME while excluding by folder name
REM robocopy exit codes 0-7 = OK, >=8 = failure
robocopy "%TOOL_DIR%" "%STAGE%\%ROOTNAME%" /E /R:1 /W:1 /NFL /NDL /NP /NJH /NJS ^
  /XD "_zip_stage" "output" "logs" "sync_backups" ".venv" "__pycache__" ".git" ^
  /XF "*.pyc" "*.log" "*.zip" >>"%LOG%" 2>&1
...
"%SEVENZIP%" a -tzip "%ZIPPATH%" "%ROOTNAME%" -r >>"%LOG%" 2>&1
set "ZERR=%ERRORLEVEL%"
