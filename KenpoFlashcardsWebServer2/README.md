...
## 🆕 Recent Updates (8.4.0 → 8.5.3.1)

### v8.5.3.1 (build 56)
- **Fixed:** Admin confirmation messages now auto-clear (no longer persist after changing user/deck).
- **Fixed:** Error confirmations use **✖** icon (not **✓**) for clarity.
- **Fixed:** Custom Set Settings → **Manage Cards** modal now stays aligned on smaller screens (input min-width override + panes stack on narrow widths).
