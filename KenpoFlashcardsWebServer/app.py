...
@app.post("/api/ai/generate_deck_preview")
def api_ai_generate_deck_preview():
    """Generate a single preview flashcard for the AI Deck Generator live example."""
    uid = current_user_id()
    if not uid:
        return jsonify({"error": "Not logged in"}), 401
    ...
    # Optional deck context to help the AI tailor output
    deck_name = str(data.get("deckName", "") or "").strip()
    deck_desc = str(data.get("deckDescription", "") or "").strip()
    extra_context = ""
    if deck_name or deck_desc:
        extra_context = f"Deck name: {deck_name}\nDeck description: {deck_desc}".strip()

    try:
        if gen_type != "keywords":
            return jsonify({"error": "Preview currently supports keywords only"}), 400

        keywords = str(data.get("keywords", "")).strip()
        if not keywords:
            return jsonify({"error": "Keywords required"}), 400

        cards = _ai_generate_from_keywords(
            keywords,
            1,
...
def _ai_generate_from_keywords(keywords: str, max_cards: int, short_answers: bool = False, instructions: str = "", extra_context: str = "") -> list:
...
Examples:
  ...
  "Nucleus" -> "Central part of a cell containing genetic material" (needs explanation)"""
    ...
    if extra_context and extra_context.strip():
        ...
        context_block = "Additional context (use if helpful):\n" + extra_context.strip() + "\n\n"


    prompt = f"""Generate exactly {max_cards} flashcards about: {keywords}\n\n{context_block}IMPORTANT: Determine the SUBJECT from the keywords ONLY. Do NOT assume any foreign language unless the keywords explicitly mention one (e.g., "Spanish", "French", "Japanese").
