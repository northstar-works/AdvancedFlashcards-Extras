...
**Current Version:** v8.6.2 (build 59)  
**Changelog:** [CHANGELOG.md](CHANGELOG.md)



## ✅ Recent FAC (v8.6.2 build 59)

### Added
- **Edit Deck modal now has tabs:** **Edit Deck** (existing) + **Edit Cards** (new).
- **Edit Cards manager:** search / group filter / select cards for the active deck (Select all, Clear selection, and # Selected counter).
- **Per-card actions:** inline **Edit** (term / definition / pron / group) and **Remove** (soft delete → deleted).
- **Restore:** restore deleted cards from **Edit Decks → Deleted** and directly from the Edit Cards list when **Show deleted** is enabled.
- **Duplicate prompt:** Add duplicate / Replace duplicate / Cancel when a term would conflict.
- **Edited history:** edited-cards history list with **Clear edited history**.
- **AI template bulk editor:** choose one or more fields (Term / Definition / Pron / Group), type instructions, see an auto 1-example preview, then **Preview All** + Apply.

### Changed
- Removed the non-action dot indicator on the deck list (active highlighting remains).
- AI/bulk edits override any “short terms only” behavior for the selected field(s).

### Fixed
- Responsive inline editor formatting (matches the “In Custom Set” list style).
- Modal scroll handling (overlay scrolls; background locked).
- Search + selections clear on close, plus an X to clear search.
- AI Template modal stacking + in-modal alerts that auto-hide.
- JS/Python runtime issues that could prevent the server from starting in some hotfix builds.
