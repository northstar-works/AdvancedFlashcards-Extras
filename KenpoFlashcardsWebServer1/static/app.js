...
async function loadVersionIntoMenu() {
  try {
    ...
    const el = document.getElementById('userVersionInfo');
    if (!el || !v) return;

    // Packaged EXE shows both versions; stand-alone webserver shows only one.
    if (v.mode === 'packaged' && v.app_version && v.web_version) {
      el.innerHTML = `App Version: ${v.app_version} (build ${v.app_build || ''})<br>WebAppServer: ${v.web_version} (build ${v.web_build || ''})`;
    } else {
      const name = v.name || 'WebAppServer';
      el.textContent = `${name}: ${v.version || 'unknown'} (build ${v.build || ''})`;
    }
  } catch (e) {
    // ignore
  }
...
async function refreshCounts(){
  try{
    // In Custom Set mode, use the custom-set counts (and avoid overwriting with /api/counts)
    if(activeTab === "custom"){
...
async function refresh(){
  ...
  if(isStudyMode){
    ...
    if(activeTab === "custom"){
      ...
      // Ensure the status line reflects Custom Set mode (and doesn't stay on "All (flat)...")
